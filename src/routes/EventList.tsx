  ...
  return (
    ...
    >
      ...
      <div className="grow min-w-0">
        <div className="block truncate w-50 sm:w-60 text-base font-bold">{name}</div>
        <div className="block truncate 40 sm:w-50 text-sm">
          <span className=""> {description}</span>
        </div>
        <div className="h-1" />
        <div className="block truncate w-:40 sm:w-50 text-sm">Date: {formatDate(start_date.toDate(), "MMMM d, yyyy")}</div>
        <div className="block truncate w-40 sm:w-50 text-sm">Location:  {location}</div>
      </div>
    </Link>
  )
}
